...
function PlayerCard({ player, groupName, arrow, adjStats, onPlayerClick }) {
  ...
  return (
    <div className={`flex-1 rounded-lg p-3 border ${arrow === "up" ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
      ...
      <p className="font-bold text-sm cursor-pointer hover:text-blue-600 transition-colors" onClick={() => onPlayerClick(player.player)}>{player.player}</p>
              ...
              {sg.pairs.map((pair, j) => (
                <div key={j}>
                  ...
                  <div className="flex gap-3 items-center">
                    <PlayerCard player={pair.moveDown} groupName={sg.harderGroup} arrow="down" adjStats={adjStats} onPlayerClick={(name) => navigate(`/player?name=${encodeURIComponent(name)}`)} />
                    <div className="text-center shrink-0">
                      <ArrowUpDown className="w-5 h-5 text-slate-400 mx-auto" />
                      <p className="text-xs text-muted-foreground mt-1">Swap</p>
                    </div>
                    <PlayerCard player={pair.moveUp} groupName={sg.easierGroup} arrow="up" adjStats={adjStats} onPlayerClick={(name) => navigate(`/player?name=${encodeURIComponent(name)}`)} />
                  </div>
                ...
                return (
                  <div key={i} className="flex items-center gap-3 bg-slate-50 rounded-lg px-3 py-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold truncate cursor-pointer hover:text-blue-600 transition-colors" onClick={() => navigate(`/player?name=${encodeURIComponent(r.player)}`)}>{r.player}</p>
